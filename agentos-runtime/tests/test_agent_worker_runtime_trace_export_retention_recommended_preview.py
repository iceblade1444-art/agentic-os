...
def test_recommended_preview_bundles_policy_preview_storage_and_recommendations_without_mutation(tmp_path):
    ...
    result = call_api(tmp_path, "/api/agent-worker/runtime-trace-export-retention/recommended-preview")
    standalone_preview = call_api(tmp_path, "/api/agent-worker/runtime-trace-export-retention/preview?max_active=10&max_archived=50&older_than_days=30")
    standalone_recommendations = call_api(tmp_path, "/api/agent-worker/runtime-trace-export-retention/recommendations")

    assert result["status"] == "ok"
    assert result["decision"] == "runtime_trace_export_retention_recommended_preview"
    ...
    assert result["preview"]["policy"] == result["recommended_policy"]
    assert result["preview"]["counts"] == standalone_preview["counts"]
    assert result["preview"]["archive_candidates"] == standalone_preview["archive_candidates"]
    assert result["preview"]["prune_candidates"] == standalone_preview["prune_candidates"]
    ...
    assert result["recommendations"]["decision"] == "runtime_trace_export_retention_recommendations"
    assert result["recommendations"]["estimated_actions"] == standalone_recommendations["estimated_actions"]
    assert result["recommendations"]["severity"] == "action_recommended"
...
def test_recommended_preview_empty_workspace_is_read_only_and_does_not_create_artifact_dirs(tmp_path):
    seed_ledgers(tmp_path)
    before_ledgers = ledger_snapshot(tmp_path)
    ...
    assert result["recommendations"]["severity"] == "empty"
    assert result["operator_next_steps"] == ["monitor_storage_summary"]
    assert ledger_snapshot(tmp_path) == before_ledgers
    assert not exports_dir(tmp_path).exists()


def test_recommended_preview_small_workspace_uses_monitor_next_step(tmp_path):
    seed_ledgers(tmp_path)
    ...
    assert result["recommendations"]["severity"] == "monitor"
    assert result["preview"]["counts"]["archive_candidates"] == 0
    assert result["preview"]["counts"]["prune_candidates"] == 0
    assert result["operator_next_steps"] == ["monitor_storage_summary"]
    assert ledger_snapshot(tmp_path) == before_ledgers
    assert all(path.exists() for path in paths)


def test_dashboard_contains_recommended_preview_action_and_markers():
    text = INDEX.read_text(encoding="utf-8")
    assert "Recommended Retention Preview" in text
    assert "loadAgentWorkerRuntimeTraceExportRecommendedPreview" in text
    assert "/api/agent-worker/runtime-trace-export-retention/recommended-preview" in text
    assert "runtime_trace_export_retention_recommended_preview" in text
    assert "operator_next_steps" in text
    assert "review_archive_candidates" in text
    assert "review_prune_candidates" in text

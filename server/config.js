...
export const config = {
  ...
  // Shared secret for the Node → Python runtime hop. Generated by deploy.sh.
  ...
  // Signs session cookies and derives the encryption keys for MFA secrets,
  // governance secrets and Google tokens. The AUTH_TOKEN fallback stays because
  // existing installs encrypted their data under it and changing the derivation
  // would make all of it unreadable — but the old last-resort literal is gone:
  // a shipped default secret means anyone can mint a valid session cookie. With
  // neither variable set, auth is disabled anyway, so a per-process random value
  // costs nothing except sessions not surviving a dev restart.
  sessionSecret: env.SESSION_SECRET || env.AUTH_TOKEN || crypto.randomBytes(32).toString("hex"),
  // True when the value above is the per-process random one. Anything encrypted
  // under an ephemeral key is unreadable after a restart, so at-rest encryption
  // is skipped in that state rather than quietly destroying what it protects.
  sessionSecretEphemeral: !(env.SESSION_SECRET || env.AUTH_TOKEN),
